...
const rpcClient = rpc.Client.$create(8545, 'localhost')

const EthereumAccountStore = require('./EthereumAccountStore')
const EthereumAccountPublisher = require('./EthereumAccountPublisher')
...
let accountFolder = config.account.folder

const accountStore = new EthereumAccountStore()
const accountPublisher = new EthereumAccountPublisher()
...
function EthereumAccountAdapter() {
  ...
  if (initialMoneyAmount === undefined ||
    ...
    throw new Error('## FATAL ERROR: Environment variable of EthereumAccountAdapter not set! Check the variables! ##')
  }
}
...
EthereumAccountAdapter.prototype.getAccountBalanceInEtherByUserId = function (userId) {
  const accountInfo = this.getAccountInfoByUserId(userId)
  ...
  try {
    const accountBalance = web3.fromWei(web3.eth.getBalance(accountAdr))
...
const newEthereumAccount = function (_userNotify) {
  const pwdNewAccount = generatePassword()
  const userNotify = _userNotify
  ...
  rpcClient.call('personal_newAccount', [pwdNewAccount], function (err, result) {
    try {
      if (err != null) {
        ...
        throw new Error('Ethereum account creation failed, there was a problem with the Ethereum client.')
      }

      const accountAdr = result
      const userId = userNotify.userId
      const userEmail = userNotify.userEmail
...
const unlockEthereumAccount = function (accountNr, passphrase, timeInSeconds) {
  ...
  files.forEach(function (file) {
    ...
    web3.eth.accounts.forEach(function (accountNr) {
      ...
      if (accountNr != null && file.indexOf(accountNr.substring(2, accountNr.length)) >= 0) {
        const accountBalance = web3.eth.getBalance(accountNr)

        // console.log('account = ' + accountNr + ", balance = " + accountBalance);
        // console.log('file path = ' + process.env.ACCOUNT_PATH);

        if (accountBalance === 0 || (removeMinAccounts && accountBalance === parseInt(config.init.account.money))) {
