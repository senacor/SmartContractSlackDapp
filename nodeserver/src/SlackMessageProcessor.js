const EthereumAccountAdapter = require('./EthereumAccountAdapter')
const EthereumLotteryAdapter = require('./EthereumLotteryAdapter')

const config = require('config')

// var adminAccountAddress = config.admin.account.address
// var adminAccountPwd = config.admin.account.password
const adminUserId = config.admin.userid

let createAccountEnabled = false

const swearArr = ['fuck', 'ass', 'ahole', 'stupid', 'hell', 'penis', 'suck', 'dick', 'shit', 'crap']
...
const isTextInMessage = function (text, message) {
  return message.indexOf(text) >= 0
}

const extracMessageWithoutUser = function (message) {
...
const processMessage = function (message) {
  const messageInfo = {}
  const noUserText = extracMessageWithoutUser(message)
...
const checkAccountInfo = function (userNotify) {
  if (userNotify.accountInfo === undefined) {
    userNotify.notifyUser('You are not registered yet! You have to run "create account" command before you can run.')
    return false
  }
  return true
}
...
const checkAdmin = function (userNotify) {
  if (userNotify.userId !== adminUserId) {
    const command = userNotify.command
...
const checkForSwearing = function (mText, userNotify) {
  for (let i = 0; i < swearArr.length; i++) {
...
const checkParams = function (userNotify, count) {
  const paramArr = userNotify.commandParams
  if (paramArr.length !== count) {
    userNotify.notifyUser(`Only exactly ${count} parameters allowed`)
    return false
  }
  return true
}
...
SlackMessageProcessor.prototype.processSlackBotMessage = function (message, userNotify) {
  ...
  const procMessage = processMessage(message)
  const mText = procMessage.text
  const mParams = procMessage.params
  ...
  const accountInfo = this.accountAdapter.getAccountInfoByUserId(userId)
  ...
  const addressToUserIdMap = this.accountAdapter.getAddressToUserIdMap()
  ...
  switch (mText) {
    ...
    case 'account':
      if (checkAccountInfo(userNotify)) {
        ...
        let balance = this.accountAdapter.getAccountBalanceInEtherByUserId(userId)
    ...
    case 'reset lottery':
      if (checkAdmin(userNotify) &&
        ...
        const minAmount = mParams[0]
        const initAmount = mParams[1]
